    ...
    <Tabs default-value="profile" class="space-y-4">
      ...
      <TabsContent value="onboard" class="space-y-4">
        <div class="rounded-xl border bg-card text-card-foreground shadow-sm p-6 space-y-6">
          ...
          <div class="space-y-4 max-w-2xl">
            <div class="space-y-2">
              <label class="text-sm font-medium leading-none">Personal Name</label>
              <input
                type="text"
                v-model="onboard.personal_name"
...
const createMerchant = async () => {
    ...
    if (!onboard.value.personal_name || !onboard.value.email || !onboard.value.password) {
    ...
    try {
        const response = await call('iswitch.admin_portal_api.onboard_merchant', {
            personal_name: onboard.value.personal_name,
        ...
        if (response && (response.success || response.message?.success)) {
            ...
            onboard.value = { personal_name: '', email: '', password: '', pancard: '' }
