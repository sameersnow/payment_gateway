<template>
  ...
  >
    <!-- Header -->
    <div class="h-16 flex items-center border-b border-border" :class="isCollapsed ? 'justify-center px-2' : 'justify-between px-4'">
      <div class="flex items-center gap-2 relative" :class="isCollapsed ? 'justify-center' : 'overflow-hidden'">
        <transition name="logo-transition" mode="out-in">
          <img 
            v-if="isCollapsed" 
            key="icon"
            src="/blinkpe-icon.png" 
            alt="BlinkPe" 
...
const navItems = [
  ...
  { path: '/transactions', label: 'Transactions', icon: BookOpen },
  ...
  { path: '/processors', label: 'Processors', icon: Server },
