...
def login():
    try:
        ...
        if not frappe.db.exists("Lien Wallet",usr):
            frappe.get_doc({
                "doctype": "Lien Wallet",
                "merchant_id": usr,
                "balance": 0
...
def signup():
    try:
        ...
        required_fields = ['personal_email','personal_name','contact_detail', 'company_name', 'password']
        ...
        if not validate_email_address(personal_email):
            return {
                "error":"Invalid email address format"
            }
        
        # Check if email already exists
        if frappe.db.exists("User", {"email": personal_email}):
            return {
        ...
        login_manager = LoginManager()
        login_manager.authenticate(user=personal_email, pwd=password)
        login_manager.post_login()
        ...
        if not frappe.db.exists("Wallet",user.name):
            wallet = frappe.new_doc("Wallet")
            wallet.merchant_id = user.name
            wallet.balance = 0
            wallet.status = 'Active'

            wallet.save(ignore_permissions=True)

        if not frappe.db.exists("Lean Wallet",user.name):
            frappe.get_doc({
                "doctype": "Lean Wallet",
